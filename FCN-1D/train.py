...
from fcn_model import FCN1DClassifier # 從當前目錄(FCN-1D)的 fcn_model.py 引入
...
if __name__ == "__main__":
    ...
    parser.add_argument('--kernel_sizes', type=str, default='7,5,3', help='Comma separated kernel sizes for FCN blocks')
    ...
    try:
        ...
        print("Error: --kernel_sizes must be comma separated integers like '7,5,3'")
        sys.exit(1)

    print(f"--- FCN-1D Training Settings ---")
    print(f"Sport: {args.sport}, Type: {args.type}")
    print(f"Kernels: {kernel_sizes}, Dropout: {args.dropout}")
    ...
    if args.sport == 'deadlift':
        ...
        save_dir = os.path.join(os.path.dirname(__file__), 'checkpoints', f'FCN_Deadlift_{feat_type}', args.tag)
        num_classes = 4
        input_len = 110
    else:
        # Benchpress
        ...
        save_dir = os.path.join(os.path.dirname(__file__), 'checkpoints', f'FCN_Benchpress', args.tag)
        num_classes = 4
        input_len = 100
    ...
    for i, (t_idx, v_idx, test_indices) in enumerate(dataset_folds):
        ...
        save_path = os.path.join(save_dir, f"FCN1D_model_fold{i}.pth")
        txt_dir = os.path.join(save_dir, f"FCN1D_model_fold{i}_results")
